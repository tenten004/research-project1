...
#直近の実験結果を表示
df_result = pd.read_csv("./data_v5.csv")
df_result.tail(5)
...
# モデルの保存
model.save(f'./model/v5/wm_cnn_{exp_num:}.h5')
...
plt.grid()
plt.savefig(f'./img/v5/accuracy/wm_accuracy_{exp_num:}.png')
#plt.show()
...
plt.legend(['train', 'test'], loc='upper left')
plt.grid()
plt.savefig(f'./img/v5/loss/wm_loss_{exp_num:}.png')
#plt.show()
...
with open('./data_v5.csv', 'a', newline='') as f:
